...
#########################################################################
#                                                                       #
#                         phpBay Pro API 2.5.1                          #
#                             Wired Studios                             #
#                         www.wiredstudios.com                          #
#                                                                       #
#########################################################################
# COPYRIGHT NOTICE                                                      #
# Copyright 2006-2008 Wired Studios, Inc.  All Rights Reserved.         #
#                                                                       #
...
#########################################################################
  ...
  $site_url = "http://www.look2vancouver.com/";
  
  # possible settings:
  # afcj = Commission Junction (no longer available on May 1st, 2008)
  # afmp = Mediaplex
  # afan = Affilinet
  # aftd = TradeDoubler
  # afepn = Ebay Partner Network
  # set for Ebay Partner Network
  $aff_type = "afepn";
  
  # enter your Ebay PID or Campaign ID
  $ebay_pid = "5335863629";
  
  # enter the Custom ID.  This is the Campaign name associated with the Campaign ID.
  # Leave as "Default campaign" if not using a custom campaign in Ebay Partner Network.
  # This field is mandatory, so leave as is, if not using custom campaigns.
  $ebay_cid = "look2vancouver";
...
class ebay{
  ...
  var $eb_sacqyop = "ge";
  var $eb_fbfmt = "";
  var $eb_sabfmts = "0"; # show all items; 1=auctions only; 2=buy it now items only
  var $eb_saprclo = ""; #min bid price
  var $eb_saprchi = ""; #max bid price
  var $eb_saaff = "afcj"; #affiliate type
  var $eb_pid = ""; #ebay pid number
  var $eb_cid = ""; #ebay custom id
  var $eb_ftrt = "1";
  var $eb_fcl = "3";
  var $eb_fts = "";
...
function listings($keywords, $category) {
  global $site_url, $ebay_pid, $aff_type, $encode, $mod_rewrite, $ebay_cid;
  
  # assign variables
  $this->eb_saaff = $aff_type;
  $this->eb_pid = $ebay_pid;
  $this->eb_cid = urlencode($ebay_cid);
  ...
  if ($this->eb_pid == "") {echo "Set the affiliate id variable in ebay.php"; exit;}
  error_reporting(0);
  ...
  $rss->get($this->eb_rss_url);
    foreach ($rss->itemInfo as $item) {
	  $count++;
	  # make sure varibles are null because we're in a foreach loop
	  unset($this->image_url);
	  unset($this->bid_now_text);
	  unset($this->bid_now_url);
	  unset($this->watch_text);
	  unset($this->watch_url);
	  unset($this->buy_now_text);
	  unset($this->buy_now_url);
	  ...
	  if (($this->eb_sid > "") && ($this->eb_saaff != "afepn")) {
...
function xml_url() {
  ...
  $url .= "fcl=" . $this->eb_fcl . "&";
  $url .= $this->eb_saaff . "=" . $this->eb_pid . "&";
  
  if ($this->eb_saaff == "afepn") {
    $url .= "customid=" . $this->eb_cid . "&";
  }
