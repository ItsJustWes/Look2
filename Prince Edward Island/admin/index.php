...
// -->
	...
	if ($searchtitle_substring == "on") {
            $query .=  "SELECT Prince Edward Island.* FROM Prince Edward Island WHERE Name like '%" . $searchtitle . "%'";
        } else {
            //exact match (faster)
            $query .=  "SELECT Prince Edward Island.* FROM Prince Edward Island WHERE Name like '%" . $searchtitle . "%'";
        }
			...
			?>" > <font size='1'>Edit </a>
                   ...
                   <?php
                    printf("<td><font size='1'> %d </font></td><td><font size='1'> %s </td><td><font size='1'> %s </td><td><font size='1'> %s </td><td><font size='1'> %s </td><td> <font size='1'>%s </td>\n",
                        $row["Id"], $row["Name"], $row["City"], $row["Telephone"], $row["Keywords1"], $row["Keywords1"]);
                printf("</tr>\n");
...
function ActionEdit($id, $searchtitle, $searchtitle_substring, $searchtitle_matches_limit)
{
    ...
    $db = @mysql_connect($DATABASE_SERVER, $DATABASE_USER, $DATABASE_PASSWORD)
        or die("<br>Error connecting to database (the server might be down): <br><b>" . mysql_error() . "</b><br>\n");
    mysql_select_db($DATABASE_NAME, $db);
    $query = "SELECT Prince Edward Island.* FROM Prince Edward Island WHERE Id=" . $id;
    $result = mysql_query($query, $db)
        or die("Error executing query: " . mysql_error());
...
<font face="Verdana" size="2"><b>Province:</b></font>
<input name="state" size=10  maxlength=255 value="">
<font face="Verdana" size="2"><b>Postal Code:</b></font>
...

{
    ...
    $query = "SELECT Id FROM Prince Edward Island WHERE Id='$id'";
    $result = mysql_query($query, $db)
        or die("Error executing query: " . mysql_error());
    $row = mysql_fetch_array($result);
    if ($row) {
        ...
        //Create record
        $query  = "INSERT INTO labrador (Name, Contact, Address, City, State, Zip, Telephone, Fax, Web, Email, Keywords1, Keywords1, Description) ";
...
function ActionDelete($selected_id, $searchname, $searchname_substring, $searchname_matches_limit)
{
	...
	echo "No records were selected for deletion..<br>\n";
	    ...
	    foreach ($selected_id as $id) {
			...
			or die("<br>Error connecting to database (the server might be down): <br><b>" . mysql_error() . "</b><br>\n");
		    mysql_select_db($DATABASE_NAME, $db);
		    $query = "DELETE FROM Prince Edward Island WHERE Id=" . $id;
		    $result = mysql_query($query, $db)
			or die("Error executing query: " . mysql_error());
...
function ActionEditSave($id, $name, $contact, $address, $city, $state, $zip, $telephone, $fax, $web, $email, $keywords1, $keywords2, $description, $searchtitle, $searchtitle_substring, $searchtitle_matches_limit)
{
    ...
    $query = "SELECT Id FROM Prince Edward Island WHERE Name='$name' and Id<>'$id'";
    $result = mysql_query($query, $db)
        or die("Error executing query: " . mysql_error());
    $row = mysql_fetch_array($result);
    if ($row) {
        ...
        //update record
       $query  = "UPDATE labrador SET Name='$name', Contact='$contact', Address='$address', City='$city', State='$state', Zip='$zip', Telephone='$telephone', Fax='$fax', Web='$web', Email='$email', Keywords1='$keywords1', Keywords1='$keywords2', Description='$description'";
       $query .= "WHERE Id=" . $id;
        $result = mysql_query($query, $db)
            or die("Error executing query: " . mysql_error());
...
function Counttitles()
{
    ...
    $query = "SELECT Count(*) as Numtitles FROM Prince Edward Island";
    $result = mysql_query($query, $db)
        or die("Error executing query: " . mysql_error());
    $row = mysql_fetch_array($result);
...
<a href="/">Search and Edit Entries</a> | <a href="recent.php">RECENT RECORDS</a>
